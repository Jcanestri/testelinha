...
class ModelTransformer:
    def __init__(self, model, first_frame, hsv_values, with_points=False):
        self.H = np.identity(3, np.float32)  #cria uma matriz identidade de float 3x3
        self.hsv_low, self.hsv_high = hsv_values #atribui os valores de HSV
        self.dst=first_frame
        self.last_frame = first_frame #carrega o primeiro frame
        self.rows, self.cols, channels = first_frame.shape #define linhas e colunas
        self.model = cv2.resize(model.image, (self.cols, self.rows))  #escala a imagem do modelo do campo para o tamanho do frame do vídeo

        frame_pts = list()
        model_pts = list()
        if with_points:       #seleciona os pontos no primeiro frame
            self.refPT = list()
    ...
    def new_frame(self, frame):
        ...
        #cv2.imshow('warped mask', warped_frame)
        #cv2.waitKey()
    ...
    def novo_frame(self, frame):
        self.idx += 1

        warped_frame = cv2.warpPerspective(frame, self.H, (self.cols, self.rows))
        alpha=0.5
        beta=1-alpha
        self.dst = cv2.addWeighted(self.model, alpha, warped_frame, beta, 0.0)
        cv2.imshow('Imagem deformada + modelo', self.dst)
        cv2.imshow('Imagem deformada', warped_frame)
        #cv2.waitKey()
